"""
Servicio principal encargado del entrenamiento,
evaluación e inferencia del modelo de clasificación
de complejidad algorítmica.

El modelo implementa una Red Neuronal Multicapa (MLP)
desarrollada completamente desde cero utilizando NumPy,
sin dependencias de frameworks de Deep Learning como
TensorFlow o PyTorch.

Objetivo
---------
Clasificar automáticamente fragmentos de código Python
en una de las siguientes clases de complejidad:
...
La clasificación se realiza a partir de las
20 características extraídas por el módulo
...
- LR Decay: 150 → 100 épocas
- Ruido de augmentación más suave
- Mayor paciencia para Early Stopping
...
class ComplexityModelService:
    ...
    def _build_balanced_dataset(self, seed=42):
        ...
        X_raw = np.array([extract_features(code) for code, _ in SAMPLES])
        ...
        split = int(len(X) * 0.8)
        return (
            X[perm[:split]], X[perm[split:]],
            y[perm[:split]], y[perm[split:]]
        )
    ...
    def train_model(self) -> dict:
        ...
        self.W1 = np.random.randn(FEATURE_SIZE, HIDDEN1) * scale(FEATURE_SIZE)
        self.b1 = np.zeros(HIDDEN1)
        self.W2 = np.random.randn(HIDDEN1, HIDDEN2) * scale(HIDDEN1)
        self.b2 = np.zeros(HIDDEN2)
        self.W3 = np.random.randn(HIDDEN2, NUM_CLASSES) * scale(HIDDEN2)
        ...
        for epoch in range(EPOCHS):
            ...
            while not batch_queue.is_empty():
                for i in batch_queue.dequeue():
                    out  = self._forward(X_train[i])
                    if np.argmax(out) == y_train[i]:
                        correct += 1
                    loss = float(np.mean((out - Y_train[i]) ** 2))
            ...
            if (epoch + 1) % 50 == 0:
                print(
                    ...
                    f"Acc: {acc:.1f}% | Loss: {avg_loss:.4f} | "
                    f"Best: {best_acc:.1f}% | LR: {lr:.5f} | "
        ...
        print(f"\nTraining complete")
        print(f"Epochs run   : {self.epochs_run}")
        print(f"Best train   : {best_acc:.1f}%")
